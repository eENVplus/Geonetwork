...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
	...
	<xsl:template name="buttons" match="*">
	 ...
	 /root/gui/schemalist/name[.=$metadata/geonet:info/schema]/@edit='true'">
		...
		or (geonet:info/isHarvested = 'n' and geonet:info/edit='true')">
			&#160;
			<button class="content" onclick="javascript:window.open('{/root/gui/config/euosmeService}?id={$metadata/geonet:info/id}&amp;locale={/root/gui/language}','EUOSME');"><xsl:value-of select="/root/gui/strings/edit_in_euosme"/></button>
			</xsl:if>
